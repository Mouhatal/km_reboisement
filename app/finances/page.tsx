...
export default function FinancesPage() {
  ...
  // État pour la suppression personnalisée
  const [deleteTarget, setDeleteTarget] = useState<{ id: string; type: 'decaissement' | 'depense' } | null>(null);
  ...
  // Modification pour utiliser modale personnalisée
  const handleDelete = (id: string, type: 'decaissement' | 'depense') => {
  ...
  const getTotalDecaissements = () => decaissements.reduce((sum, d) => sum + d.montant, 0);
  const getTotalDepenses = () => depenses.reduce((sum, d) => sum + d.montant, 0);
  const getSolde = () => getTotalDecaissements() - getTotalDepenses();
    ...
    <div className="min-h-screen bg-gray-50">
          ...
          <div className="border-b">
            <div className="flex">
              <button
                ...
                className={`px-6 py-4 font-medium ${activeTab === 'decaissements' ? 'border-b-2 border-green-600 text-green-600' : 'text-gray-500 hover:text-gray-700'}`}
              >
                Décaissements ({decaissements.length})
              </button>
              <button
                onClick={() => setActiveTab('depenses')}
                className={`px-6 py-4 font-medium ${activeTab === 'depenses' ? 'border-b-2 border-green-600 text-green-600' : 'text-gray-500 hover:text-gray-700'}`}
              >
          ...
          <div className="p-4">
            {loadingData ? (
              ...
              <DecaissementsTable
                ...
                onEdit={(item: Decaissement) => { setEditingItem(item); setShowForm(true); }}
              ...
              <DepensesTable
                depenses={depenses}
                onEdit={(item: Depense) => { setEditingItem(item); setShowForm(true); }}
      ...
      {showForm && (
        <FinanceForm
          type={activeTab}
          item={editingItem}
          onClose={() => { setShowForm(false); setEditingItem(null); router.replace('/finances', undefined); }}
          onSave={() => { setShowForm(false); setEditingItem(null); loadFinances(); router.replace('/finances', undefined); }}
        />
      )}

      {/* Modale de confirmation suppression personnalisée */}
      {deleteTarget && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-lg max-w-md w-full p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Confirmation de suppression</h3>
            <p className="text-gray-700 mb-6">
              Êtes-vous sûr de vouloir supprimer ce {deleteTarget.type === 'decaissement' ? 'décaissement' : 'dépense'} ?
            </p>
...
// === Les tables et form restent inchangées ===
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {decaissements.map((item: Decaissement) => (
            <tr key={item.id} className="hover:bg-gray-50">
              <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-900">{format(new Date(item.date), 'dd/MM/yyyy')}</td>
              <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-900">{item.reference_bancaire}</td>
              <td className="px-4 py-3 text-sm text-gray-600">{item.description || '-'}</td>
              <td className="px-4 py-3 whitespace-nowrap text-sm text-right font-semibold text-blue-600">{item.montant.toLocaleString()} FCFA</td>
              <td className="px-4 py-3 text-sm text-gray-600">
                {item.recus && item.recus.length > 0 ? (
                  <div className="flex flex-wrap gap-1">
                    {item.recus.map((url: string, index: number) => (
                      <a key={`recu-${index}`} href={url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center space-x-1 px-2 py-1 bg-blue-50 text-blue-700 rounded-full text-xs font-medium hover:bg-blue-100">
                        <FileText size={14} /><span>Reçu {index + 1}</span>
                      </a>
                    ))}
                  </div>
                ) : '-'}
              ...
              <td className="px-4 py-3 whitespace-nowrap text-sm text-right">
                <div className="flex items-center justify-end space-x-2">
                  <button onClick={() => onEdit(item)} className="p-2 text-blue-600 hover:bg-blue-50 rounded" title="Modifier"><Edit size={16} /></button>
                  {canDelete && <button onClick={() => onDelete(item.id)} className="p-2 text-red-600 hover:bg-red-50 rounded" title="Supprimer"><Trash2 size={16} /></button>}
                </div>
              </td>
            </tr>
          ))}
...
function DepensesTable({ depenses, onEdit, onDelete, canDelete }: any) {
  if (depenses.length === 0) return <div className="text-center py-12 text-gray-500">Aucune dépense enregistrée</div>;
  return (
    <div className="overflow-x-auto">
      <table className="min-w-full divide-y divide-gray-200">
        <thead className="bg-gray-50">
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {depenses.map((item: Depense) => (
            <tr key={item.id} className="hover:bg-gray-50">
              ...
              <td className="px-4 py-3 whitespace-nowrap text-sm text-right font-semibold text-red-600">{item.montant.toLocaleString()} FCFA</td>
              <td className="px-4 py-3 text-sm text-gray-600">
                {item.recus && item.recus.length > 0 ? (
                  <div className="flex flex-wrap gap-1">
                    {item.recus.map((url: string, index: number) => (
                      <a key={`recu-${index}`} href={url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center space-x-1 px-2 py-1 bg-red-50 text-red-700 rounded-full text-xs font-medium hover:bg-red-100">
                        <FileText size={14} /><span>Reçu {index + 1}</span>
                      </a>
                    ))}
                  </div>
